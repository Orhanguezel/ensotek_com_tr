...
import {
  getSmtpSettings,
} from "@ensotek/shared-backend/modules/siteSettings";
...
export async function sendDepositSuccessMail(input: DepositSuccessMailInput) {
  const d = depositSuccessMailSchema.parse(input);
  const subject = `Bakiye Yuklemesi — ${SITE_NAME}`;
  const html = wrapMailBody(`
    <h2 style="font-size:18px;">Bakiye Yuklemesi Tamamlandi</h2>
    <p>Merhaba <strong>${escapeMailHtml(d.user_name)}</strong>,</p>
    <p>Hesabiniza <strong>${escapeMailHtml(d.amount)}</strong> tutarinda yukleme yapildi.</p>
    <p>Guncel bakiye: <strong>${escapeMailHtml(d.new_balance)}</strong></p>
    <p>${SITE_NAME} Ekibi</p>
  `);
  const text = `Merhaba ${d.user_name},\n\nHesabiniza ${d.amount} yukleme yapildi. Guncel bakiye: ${d.new_balance}\n\n${SITE_NAME} Ekibi`;
  return sendMailRaw({ to: d.to, subject, html, text });
}
