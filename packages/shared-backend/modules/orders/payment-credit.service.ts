...
import { dealerProfiles, dealerTransactions } from '@ensotek/shared-backend/modules/dealerFinance';
import { orders, orderItems } from '@ensotek/shared-backend/modules/orders';
...
export async function finalizeOrderPaymentWithDealerCredit(params: {
  ...
  await db.transaction(async (tx) => {
    ...
    if (order.payment_status === 'pending') {
      throw new CreditPaymentError('payment_already_in_progress');
    }
