...
export function isGlobalKey(key: string): boolean {
  ...
  // Site media + app icons + theme assets are stored locale='*'
  if (
    normalizedKey === 'site_logo' ||
    normalizedKey === 'site_logo_dark' ||
    normalizedKey === 'site_logo_light' ||
    normalizedKey === 'site_favicon' ||
    normalizedKey === 'site_apple_touch_icon' ||
    normalizedKey === 'site_app_icon_512' ||
    normalizedKey === 'site_og_default_image'
  ) return true;
  if (normalizedKey.startsWith('app_icon_')) return true;
  return false;
}
