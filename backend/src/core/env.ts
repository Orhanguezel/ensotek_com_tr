...
export const env = {
  ...
  CLOUDINARY_FOLDER: process.env.CLOUDINARY_FOLDER || 'ensotek-com-tr',
  ...
  CLOUDINARY: {
    ...
    folder: process.env.CLOUDINARY_FOLDER || 'ensotek-com-tr',
  ...
  DB: {
    ...
    name: process.env.DB_NAME || 'ensotek_com_tr_db',
  },
  ...
  CORS_ORIGIN,
  FRONTEND_URL,
  PUBLIC_URL: process.env.PUBLIC_URL || 'https://www.ensotek.com.tr',

  SITE_SETTINGS_KEY_PREFIX: (() => {
    const raw = process.env.SITE_SETTINGS_KEY_PREFIX;
    if (raw === undefined || raw === null) return 'ensotek_com_tr__';
    const t = String(raw).trim();
    if (t === '' || t === '-') return 'ensotek_com_tr__';
  ...
  AI_SUPPORT_ENABLED: toBool(process.env.AI_SUPPORT_ENABLED, false),
